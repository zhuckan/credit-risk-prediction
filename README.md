...
- **Target Variable (`flag`)**: A binary indicator where `1` signifies a default event. The dataset is highly imbalanced, with positive default cases accounting for approximately **3.55%** of the data.

## Methodology

### 1. Data Preprocessing and Aggregation
Due to the large size of the source data, files are processed iteratively. For each chunk:
...
The repository contains the main notebook with the code and explanatory comments.
...
To explore the problem-solving approach, simply open the good_job.ipynb file directly on GitHub or download it and view it in any text editor that supports Jupyter Notebooks (e.g., VS Code). The notebook thoroughly details each step of the pipeline, from data loading and aggregation to model building and evaluation.
